...
class EmailnotificationsRequestValidationTests {
	...
	void whenIsbnNotDefinedThenValidationFails() {
		...
		assertThat(violations.iterator().next().getMessage())
				.isEqualTo("The subscription ISBN must be defined.");
	}
	...
	void whenQuantityIsNotDefinedThenValidationFails() {
		...
		assertThat(violations).hasSize(1);
		assertThat(violations.iterator().next().getMessage())
				.isEqualTo("The subscription quantity must be defined.");
	}
	...
	void whenQuantityIsLowerThanMinThenValidationFails() {
		...
		assertThat(violations).hasSize(1);
		assertThat(violations.iterator().next().getMessage())
				.isEqualTo("You must emailnotifications at least 1 item.");
	}
	...
	void whenQuantityIsGreaterThanMaxThenValidationFails() {
		...
		assertThat(violations).hasSize(1);
		assertThat(violations.iterator().next().getMessage())
				.isEqualTo("You cannot emailnotifications more than 5 items.");
	}
}